...
class TrainingProgress(Base):
    ...
    best_genome_timestamp = Column(
        Float, default=0.0
    )  # store timestamp when best genome was found


# SQLite engine
engine = create_engine(
    "sqlite:///training.db", connect_args={"check_same_thread": False}
)
...
def save_progress(progress):
    ...
    if "best_genome" in progress and progress["best_genome"]:
        ...
        record.best_genome_timestamp = progress.get(
            "best_genome_timestamp", time.time()
        )
    session.add(record)
    session.commit()
    session.close()
...
def load_progress():
    ...
    if record:
        progress = {
            ...
            "loss_history": pd.read_json(StringIO(record.loss_history))
            if record.loss_history
            else pd.DataFrame({"loss": []}),
            "ga_history": pd.read_json(StringIO(record.ga_history))
            if record.ga_history
            else pd.DataFrame({"best_accuracy": [], "avg_accuracy": []}),
            "best_genome": json.loads(record.best_genome)
            if record.best_genome != "{}"
            else None,
